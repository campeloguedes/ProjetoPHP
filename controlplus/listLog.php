...
<script type='text/javascript'>
  ...
  function datagrid(like, order, limit) { 
	   $.ajax({ 
			 ...
			 data: 'classe=LogController&acao=datagrid&objeto=Log&like='+like+'&order='+order+'&limit='+limit, 
			 beforeSend: function() { }, 
  ...
  var page		 = 1; 
  var defaultLike  = ""; 
  var defaultOrder = 'idlog'; 
  var auxOrder = 'idlog'; 
  var defaultLimit = '0,10'; 
...
<div id='Content'> 
   ...
   <div class='category'>
		...
		<span class='sub_title'>Listagem De Logs</span> 
   </div>
 ...
 <br clear='all' /> 
   ...
   <div class="add" style="visibility: hidden;"><label><input onclick='window.location="<?php echo $mainFolder; ?>/controlplus/cadLog";' type="image" src="<?php echo $mainFolder; ?>/controlplus/includes/imgs/icon-add.png" /> <a href="<?php echo $mainFolder; ?>/controlplus/cadLog">Novo Registro</a> </label><label><input onclick='controlDel2();' type="image" src="<?php echo $mainFolder; ?>/controlplus/includes/imgs/icon-del.png" /> <a onclick='controlDel2();'>Excluir Selecionado(s)</a> </label></div>

   <table id='listReg' cellspacing='1' class='table'>
     <thead> 
        <tr> 
		
         <th class="order" id="th_idlog" onclick="ordena('idlog');">ID</th>
         <th class="order" id="th_dt" onclick="ordena('dt');">Data</th>
         <th class="order" id="th_tabela" onclick="ordena('tabela');">Tabela</th>
         <th class="order" id="th_chave" onclick="ordena('chave');">Chave</th>
         <th class="order" id="th_acao" onclick="ordena('acao');">Açao</th>
         <th class="order" id="th_ip" onclick="ordena('ip');">Ip</th>
         <th class="order" id="th_tipo" onclick="ordena('tipo');">Tipo</th>
         ...
         <tfoot> 
			  <tr><th colspan="50" id="rows"><div style="float:left; margin-top:7px; color:#333;">Total de registros: <span style="color:#EE0000; font-weight:bold;" id="totalRegs">&nbsp;</span></div><div style="float:right"><img id="cpload2" style="float:left; margin:3px 12px;" src="<?php echo $mainFolder; ?>/controlplus/includes/imgs/cpload.gif"/><label style="float:left; padding-top:7px;">Exibir:</label><select onchange="focusTop(); $('#cpload2').show(); limit1 = 0; page = 1; prepare();" class="geral" style="margin-left:5px; height:25px;" id="limit2" name="limit2"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select><label style="margin-left:15px; text-transform: lowercase;  float:left; margin-top:8px; margin-right:5px;"><span id="pageNow">&nbsp;</span> de <span id="pageTotal">&nbsp;</span></label><input onclick="$('#cpload2').show(); previousPage();" type="image" src="<?php echo $mainFolder; ?>/controlplus/includes/imgs/button_previous.gif" style="margin-left:5px; margin-top:5px;" /><input onclick="$('#cpload2').show(); nextPage();" type="image" src="<?php echo $mainFolder; ?>/controlplus/includes/imgs/button_next.gif" style="margin-left:5px; margin-top:5px;" /></div></th></tr>
		  </tfoot>
   ...
   <form id='formDel' name='formDel' action='<?php echo $mainFolder; ?>/mediaplus/controller/mpcontroller' method='post' accept-charset='utf-8'>
       <input type='hidden' name='classe' id='classe' value='LogController' />
       <input type='hidden' name='acao' id='acao' value='excluir' />
       <input type='hidden' name='objeto' id='objeto' value='Log' />
   ...
   <form id='formDel2' name='formDel2' action='<?php echo $mainFolder; ?>/mediaplus/controller/mpcontroller' method='post' accept-charset='utf-8'>
       <input type='hidden' name='classe' id='classe2' value='LogController' />
       <input type='hidden' name='acao' id='acao2' value='excluirEmMassa' />
       <input type='hidden' name='objeto' id='objeto2' value='Log' />
