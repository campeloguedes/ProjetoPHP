...
<script type='text/javascript'>
  ...
  function response(data){
      data = remover_espacos(data);	   $('#cpload1').hide();
	   ...
	   $('.btn2').show();
	   if(data==1){
		var _oEditor_paginatexto_texto = CKEDITOR.instances.paginatexto_texto;
		_oEditor_paginatexto_texto.setData("");
		 $('#formCad').resetForm();
         document.getElementById('info').innerHTML = '<p>Salvo com sucesso!</p>'; 
         $('#info').dialog('open'); 
		  window.location = '<?php echo $mainFolder; ?>/controlplus/orderPaginatexto';
	   }
	   if(data==2){
         document.getElementById('info').innerHTML = '<p>Salvo com sucesso!</p>'; 
         $('#info').dialog('open'); 
...
<script type="text/javascript">$(function(){
	...
	$('#paginatexto_texto').ckeditor(config_paginatexto_texto);
});					

function getpaginatexto_texto(){	
			var _oEditor = CKEDITOR.instances.paginatexto_texto;
...
<div id='Content'> 
   <div class='category'>
		<span class='title'>Página De Texto</span> 
		<img src="<?php echo $mainFolder; ?>/controlplus/includes/imgs/seta_indic.gif" alt='Info' /> 
		<span class='sub_title'>Cadastro De Página De Texto</span> 
   </div>
 ...
 <br clear='all' /> 
   ...
   <form class='formular' id='formCad' name='formCad' action='<?php echo $mainFolder; ?>/mediaplus/controller/mpcontroller' method='post' accept-charset='utf-8'>
       ...
       <input type='hidden' id='idpaginatexto' name='idpaginatexto' value='<?php echo $paginatexto->idpaginatexto; ?>' />

       <label>
       	   Título:<br/>
       	   <input type='text' name='titulo' id='titulo' value='<?php echo $paginatexto->titulo; ?>' /><br/><br/>
       </label>
       ...
       <div style='float:left;' onclick='oEditor = CKEDITOR.instances.paginatexto_texto; largura_img_left = 400; largura_img_all = 800;'>
<textarea name='paginatexto_texto' id='paginatexto_texto'><?php echo html_entity_decode($paginatexto->texto); ?></textarea>
<br/><br/>
</div>
       

       	   <input type='hidden' name='posicao' id='posicao' value='<?php echo $paginatexto->posicao == '' ? 0 : $paginatexto->posicao; ?>' />
       <br/>
		<div class='area_btns'>
			<input type='submit' onclick='getpaginatexto_texto(); ' style='background:url(<?php echo $mainFolder; ?>/controlplus/includes/imgs/button_salvar.gif) no-repeat; width:101px; height:36px;' class='btn1' alt='Salvar' value='' />
