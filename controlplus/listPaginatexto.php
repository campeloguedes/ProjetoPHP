...
<script type='text/javascript'>
  ...
  function datagrid(like, order, limit) { 
	   $.ajax({ 
			 type: 'POST', 
			 url: '<?php echo $mainFolder; ?>/mediaplus/controller/mpcontroller', 
			 data: 'classe=PaginatextoController&acao=datagrid&objeto=Paginatexto&like='+like+'&order='+order+'&limit='+limit, 
			 beforeSend: function() { }, 
  ...
  var page		 = 1; 
  var defaultLike  = ""; 
  var defaultOrder = 'idpaginatexto'; 
  var auxOrder = 'idpaginatexto'; 
...
<div id='Content'> 
   ...
   <div class='category'>
		...
		<span class='sub_title'>Listagem De Páginas De Texto</span> 
   </div>
 ...
 <br clear='all' /> 
   ...
   <table id='listReg' cellspacing='1' class='table'>
     <thead> 
        <tr> 
         ...
         <th class="order" id="th_idpaginatexto" onclick="ordena('idpaginatexto');">ID</th>
         <th class="order" id="th_titulo" onclick="ordena('titulo');">Título</th>
         <th class="order" id="th_posicao" onclick="ordena('posicao');">Posição</th>
         <th>Ações</th>
        </tr> 
     </thead> 
         <tbody id='datagrid'> 
   ...
   <form id='formDel2' name='formDel2' action='<?php echo $mainFolder; ?>/mediaplus/controller/mpcontroller' method='post' accept-charset='utf-8'>
       <input type='hidden' name='classe' id='classe2' value='PaginatextoController' />
       <input type='hidden' name='acao' id='acao2' value='excluirEmMassa' />
       <input type='hidden' name='objeto' id='objeto2' value='Paginatexto' />
